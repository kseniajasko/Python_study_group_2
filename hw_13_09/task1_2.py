...
def print_student(student):
    for field in student:
        field_1 = field.replace("_", " ").title()
        print(f'{field_1}: {student[field]}')
...
def dump_studens():
    with open(data_path+'student_data.json', 'w') as file:
        json.dump(STUDENTS, file)


def dump_csv():
    with open(data_path +'student_data.csv', 'w') as file:
...
def load_csv():
    with open(data_path + 'student_data.csv', 'r') as file:
...
def load_from_json(file_path=data_path+'student_data.json'):
